...
class DistanceFromGroundChecking:
    ...
    def _computeDistanceFromGround(self, joint_index):
        ...
        return abs(
            distanceBtwTwoJoint
            * (math.sin(math.radians(alphaAngle)) / math.sin(math.radians(gammaAngle)))
        )
    ...
    def checkingDistanceFromGround(self):
        ...
        for (keyReal, dReal), (keyMin, dMin) in zip(
            distancesFromTheGround.items(), self.safeDistancesFromTheGround.items()
        ):
            print(keyReal, dReal)
...
angleTest1 = [0.9509, -1.6623, 0.6353, -0.5976, -1.5722, 0.0]
angleTest2 = [0.9019, -0.28, 1.7852, -3.0774, -1.5697, -0.6689]
